...
try:
    ...
    CLOB_OK = True
    print("✅ py_clob_client_v2 OK", flush=True)
...
async def execute_trade(token_id, direction, price, size_dollars):
    ...
    try:
        ...
        plog(f"✅ ORDRE RÉEL | {direction} | {order_price} | {order_size} | {resp}")
        return resp
    except Exception as e:
...
async def trading_loop():
    ...
    mode_str = "📝 PAPER MODE" if PAPER_MODE else "🔴 TRADING RÉEL"
        ...
        try:
            ...
            if current_win != last_window:
                ...
                if position is not None:
                    ...
                    if poly_price:
                        ...
                        pnl_trade = (poly_move if position["dir"] == "UP"
                                     else -poly_move) * STAKE - (STAKE * 0.02)
                ...
                plog(f"🕐 Fenêtre | K: ${btc_kraken:,.2f} | "
                     f"Strike: {cl_str} | PnL: ${pnl_session:+.2f}")
                continue

            # Circuit breaker
            if pnl_session <= MAX_LOSS_SESSION:
                if now % 300 == 0:
                    plog(f"🔴 CIRCUIT BREAKER | Session: ${pnl_session:.2f}")
                continue
            ...
            if position is not None:
                ...
                if (elapsed >= TAKE_PROFIT_TIME or poly_loss or
                        kraken_reversed or seconds_left <= 5):
                    reason = (
                    ...
                    emoji = "✅" if pnl_trade >= 0 else "❌"
                    plog(f"")
                    plog(f"{emoji} SORTIE #{len(trades_history)} | {reason} | "
                         f"Dir: {position['dir']} | "
                         f"{position['entry_price']:.3f} → {poly_price:.3f} | "
                         f"PnL: ${pnl_trade:+.2f} | "
                         f"Session: ${pnl_session:+.2f} | "
            ...
            if now - last_log_30s >= 30:
                ...
                if p_theo is not None and diff is not None:
                    k_prices = [p for t, p in kraken_history if now - t <= 5]
                    ...
                    plog(f"📡 Poly: {poly_price:.3f} {zone_ok} | "
                         f"Écart: {ecart_ok} | "
                         f"K move: {move_ok} | {seconds_left}s")
            ...
            recent_10s = [(t, p) for t, p in kraken_history if now - t <= 10]
            if len(recent_10s) < 2:
                continue
            pct_10s = (recent_10s[-1][1] - recent_10s[0][1]) / recent_10s[0][1]
            ...
            entry_price = poly_price if direction == "UP" else (1 - poly_price)

            plog(f"")
            plog(f"🎯 SIGNAL #{len(trades_history)+1} | "
                 f"Dir: {direction} | K: {pct*100:+.3f}% | "
                 f"Poly: {poly_price:.3f} | P_théo: {p_theo:.3f} | "
                 f"Écart: {diff:+.3f} | {seconds_left}s")
...
async def stats_report():
    while True:
        try:
            ...
            mode_str = "📝 PAPER" if PAPER_MODE else "🔴 RÉEL"
            plog(f"{'='*60}")
            plog(f"📊 RAPPORT {mode_str} — {datetime.now().strftime('%H:%M')}")
            plog(f"{'='*60}")
