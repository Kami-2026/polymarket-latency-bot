...
def compute_cross_correlation():
    ...
    cl_times  = [t for t, _ in chainlink_history]
    cl_prices = [p for _, p in chainlink_history]
    ...
    for X in range(0, 61):
        ...
        for t_cl, p_cl in zip(cl_times, cl_prices):
            target     = t_cl - X
            candidates = [
                (abs(t_k - target), p_k)
                for t_k, p_k in zip(k_times, k_prices)
                if abs(t_k - target) < 3
            ]
            if candidates:
                p_k = min(candidates, key=lambda x: x[0])[1]
                kraken_aligned.append(p_k)
                chainlink_aligned.append(p_cl)

        if len(kraken_aligned) < 5:
            continue

        ecarts    = [abs(a - b) for a, b in
                     zip(kraken_aligned, chainlink_aligned)]
        moy_ecart = sum(ecarts) / len(ecarts)

        try:
            r, _ = pearsonr(kraken_aligned, chainlink_aligned)
            r2   = r ** 2
        except:
            r2 = 0

        amp_k  = max(kraken_aligned) - min(kraken_aligned)
        amp_cl = max(chainlink_aligned) - min(chainlink_aligned)
    ...
    k_moves  = 0
    cl_moves = 0
    for i in range(1, len(cl_prices)):
        k_ref = [p for t, p in zip(k_times, k_prices)
                 if abs(t - (cl_times[i] - best_X)) < 3]
        k_ref_prev = [p for t, p in zip(k_times, k_prices)
                      if abs(t - (cl_times[i-1] - best_X)) < 3]
        if k_ref and k_ref_prev:
            k_dir  = k_ref[0] - k_ref_prev[0]
            cl_dir = cl_prices[i] - cl_prices[i-1]
            if abs(k_dir) > 1:
                k_moves += 1
                if k_dir * cl_dir > 0:
                    cl_moves += 1
...
async def cross_correlation_loop():
    ...
    await asyncio.sleep(120)  # attend 2 min de données
    ...
    while True:
        ...
        if result:
            ...
            plog(f"")
            if best_r2 > 0.98 and best_result and abs(best_result[3]-1) < 0.10:
...
async def observe_loop():
    ...
    while True:
        ...
        if now % 30 == 0:
            ...
            cl_str   = (f"${btc_chainlink:,.2f}"
                        if btc_chainlink else "N/A")
            k_cl_str = f"${k_cl_diff:.2f}" if k_cl_diff else "N/A"
            theo_str = f"{p_theo:.3f}" if p_theo else "N/A"
            diff_str = f"{diff:+.3f}" if diff is not None else "N/A"
            plog(f"📡 K: ${btc_kraken:,.2f} | "
                 f"CL: {cl_str} | Δ K/CL: {k_cl_str} | "
            ...
            if direction and abs(pct) >= 0.0002:
                ...
                if not already:
                    ...
                    plog(f"🔍 MOVE {direction} {pct*100:+.3f}% | "
                         f"Poly: {poly_price:.3f} | "
                         f"P_théo: {p_theo:.3f if p_theo else 'N/A'} | "
                         f"Écart: {diff:+.3f if diff else 'N/A'} | "
                         f"Δ K/CL: ${k_cl_diff:.2f if k_cl_diff else 0:.2f} | "
                         f"{seconds_left}s")
        ...
        for move in pending_moves:
            ...
            if move["t_poly_moved"] is not None:
                ...
                poly_reversed = \
                    (move["dir"] == "UP"  and poly_move_from_lag < -0.01) or \
                ...
                if poly_reversed or elapsed_since_lag > 30:
                    ...
                    plog(f"✅ #{len(lag_measures)} | "
                         ...
                         f"Écart parieurs: "
                         f"{move['diff_entry']:+.3f if move['diff_entry'] else 'N/A'} | "
                         f"Δ K/CL: "
                         f"${move['k_cl_diff']:.2f if move['k_cl_diff'] else 0:.2f} | "
...
async def main():
    ...
    plog(f"Corrélation Kraken/Chainlink active — résultats dans 2 min")
    plog("-" * 60)
